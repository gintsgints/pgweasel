...
use crate::{convert_args::FileWithPath, parsers::{LogLine, LogParser}};
...
impl LogParser for LogLogParser {
    ...
    fn parse(mut self, min_severity: i32, mask: Option<String>, begin: Option<DateTime<Local>>, end: Option<DateTime<Local>>) -> Self::Iter {
        let iter = self.reader.lines().filter_map(move |lin| {
            ...
            Some(Ok(LogLine { pg_log: None, raw: line }))
        });
        Box::new(iter)
    }
}
...
mod tests {
    ...
    fn test_csv_parser() -> Result<()> {
        ...
        let iter = parser.parse(intseverity, Some("2025-05-21 13:00:03.127".to_string()), None, None);
        for line in iter {
            let line = line?;
            println!("{:?}", line);
        }
